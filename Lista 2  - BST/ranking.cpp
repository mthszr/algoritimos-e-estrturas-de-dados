// https://iudex.io/problem/6167437897713f0001036f42
...
private:
    ...
    {
        if (!node) // se a posição atual estiver vazia
        {                                                
        ...
        else if (name == node->name) // se o nome já existir
        {                                                  
            cout << name << " ja esta no sistema." << endl; // imprime mensagem de duplicação
        }
        else if (score < node->score) // se a pontuação for menor que a pontuação do nó atual
        {                                           
            addNode(node->left, node, name, score); // adiciona recursivamente à subárvore esquerda
        }
        else // se a pontuação for maior ou igual à pontuação do nó atual
        {                                            
    ...
    {
        if (!node || node->score == score) // se o nó for nulo ou a pontuação for encontrada
    ...
    {
        if (node->right) // se existir subárvore direita
            return findMin(node->right); // encontra o mínimo nela
    ...
    {
        if (node->left) // se existir subárvore esquerda, encontra o máximo nela
...
int main()
{
    ...
    for (int i = 0; i < K; ++i) // loop para processar cada comando
    {                          
        string command; // variável para armazenar o comando
        getline(cin, command); // lê o comando da entrada

        if (command.substr(0, 3) == "ADD") // se o comando for ADD
        {                                                       
        ...
        else if (command.substr(0, 4) == "PROX") // se o comando for PROX
        {                                               
            int score; // variável para armazenar a pontuação                                 
