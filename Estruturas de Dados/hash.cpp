...
class HashTableAberta {
   private:
    ...
    int hashFunction(int chave) {
        return ((chave ^ 2) + (30 * chave)) % tamanho;  // função hash simples
    }
   ...
   public:
    ...
    void inserirItem(int chave) {
        ...
        tabela[index] = chave;  // insere a chave no índice
...
int main() {
    HashTableAberta h(20);

    // 10, 50, 90, 34, 56, 30, 21, 34, 492, 100
    h.inserirItem(10);
    h.imprimirComoArray();
    h.inserirItem(20);
    h.imprimirComoArray();
